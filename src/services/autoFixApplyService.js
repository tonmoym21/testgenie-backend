...
/**
 * Compute diff stats (lines added / removed) from a unified-diff string.
 * Counts only body lines starting with '+' or '-', skipping the '+++' /
 * '---' header lines. Returns {plus, minus, files: 1} — files is a
 * placeholder because today every patch touches exactly one spec; if/when
 * multi-file patches land, the count should come from the diff header.
 */
function diffStats(diffText) {
  if (!diffText) return { plus: 0, minus: 0, files: 0 };
  let plus = 0;
  let minus = 0;
  ...
  return { plus, minus, files: 1 };
}

function buildPrBody(row, fileRel) {
  const stats = diffStats(row.patch_diff);
  const lines = [
    '## Auto-fix proposal from TestForge',
    '',
    `Patches \`${fileRel}\` after failure #${row.test_failure_id} (\`${row.failure_signature || 'no-signature'}\`).`,
    '',
    `**Diff:** \`${stats.files}\` file changed, \`+${stats.plus}\` / \`-${stats.minus}\` lines.`,
  ];

  // The LLM's own description of what it did and why. Captured in
  // fix_attempts.explanation (migration 022). Older rows that predate
  // the column have NULL — skip the section entirely rather than printing
  // an empty "Why this fix" header that looks like a templating bug.
  if (row.explanation) {
    lines.push('', '### Why this fix', row.explanation.toString().slice(0, 2000));
  }
  ...
  lines.push(
    ...
    `- Only \`${fileRel}\` was patched. No app source, config, or other specs were touched.`,
    `- If the failure stems from app code rather than spec code, this patch will mask it — verify the fix actually addresses the root cause before merging.`,
    '',
...
module.exports = { applyFix, buildCommitMessage, buildPrBody, diffStats };
