...
/**
 * Create an automation asset from generated Playwright tests.
 */
...
/**
 * List automation assets for a project with filters.
 */
async function listAssets(projectId, userId, { status, category, search, page = 1, limit = 20 } = {}) {
  ...
  if (category) {
    ...
    where += ` AND $${params.length} = ANY(a.categories)`;
  }
  if (search) {
    params.push(`%${search}%`);
  ...
  const countRes = await db.query(
    `SELECT COUNT(*) FROM automation_assets a JOIN projects p ON p.id = a.project_id ${where}`,
    params
  );
  ...
  const result = await db.query(
    `SELECT a.*, 
            (SELECT COUNT(*) FROM playwright_runs r WHERE r.automation_asset_id = a.id) AS run_count
...
/**
 * Update categories, tags, status, name, description.
 */
async function updateAsset(assetId, userId, updates) {
  ...
  const allowed = ['name', 'description', 'categories', 'tags', 'status'];
  const sets = [];
  const params = [assetId];

  for (const key of allowed) {
    if (updates[key] !== undefined) {
      params.push(key === 'categories' || key === 'tags' ? updates[key] : updates[key]);
  ...
  const result = await db.query(
    `UPDATE automation_assets SET ${sets.join(', ')} WHERE id = $1 RETURNING *`,
    params
  );
  return result.rows[0];
}
...
/**
 * Update last run info on an asset.
 */
async function updateLastRun(assetId, status, timestamp) {
  await db.query(
    'UPDATE automation_assets SET last_run_at = $2, last_run_status = $3 WHERE id = $1',
    [assetId, timestamp, status]
  );
}
