...
// ---------------------------------------------------------------------------
// POST /assets — create automation asset
// ---------------------------------------------------------------------------
...
// POST /assets/:assetId/verify-readiness
...
// GET /assets/:assetId/readiness — get latest validation
...
// POST /bulk-verify-readiness
const bulkVerifySchema = z.object({
  assetIds: z.array(z.number()).min(1).max(50),
});
...
// POST /assets/:assetId/run — run with readiness gate
...
router.post('/assets/:assetId/run', validate(runSchema), async (req, res, next) => {
  try {
    ...
    // Readiness gate (unless explicitly skipped)
    ...
    if (!req.body.skipPreflight) {
      ...
      if (!readinessResult || !readinessResult.preflight.ready) {
        return res.status(422).json({
          ...
          preflight: readinessResult?.preflight || null,
          validation: readinessResult?.validation || null,
        });
      }
...
// POST /bulk-run — bulk execution with readiness gate
...
router.post('/bulk-run', validate(bulkRunSchema), async (req, res, next) => {
  try {
    ...
    // Verify readiness for all assets
    ...
    if (readyIds.length === 0) {
      return res.status(422).json({
        error: { code: 'ALL_BLOCKED', message: 'No assets passed readiness verification.' },
    ...
    if (blockedIds.length > 0 && !req.body.runReadyOnly) {
      return res.status(422).json({
        error: { code: 'SOME_BLOCKED', message: `${blockedIds.length} asset(s) blocked. Set runReadyOnly=true to run only ready assets.` },
        readyIds,
        blockedIds,
        readiness: readinessResults,
      });
    ...
    const runs = await playwrightRunnerService.bulkRunWithItems(readyIds, userId, {
      ...req.body,
      projectId: parseInt(projectId, 10),
      readinessResults,
    });
...
router.get('/assets/:assetId/runs', async (req, res, next) => {
  try {
    ...
    const result = await playwrightRunnerService.getRunsForAsset(
      parseInt(req.params.assetId, 10), req.user.id,
...
// GET /runs/:runId/items — execution run items for bulk runs
...
// GET /executions — list all execution runs for project
router.get('/executions', async (req, res, next) => {
  try {
    ...
    const params = [parseInt(projectId, 10), userId];
    ...
    const countRes = await db.query(
      `SELECT COUNT(*) FROM playwright_runs r JOIN projects p ON p.id = r.project_id ${where}`, params
    );
